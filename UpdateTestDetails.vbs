...
'********************************************************************************************
'Writing to system variables require elevated privileges, so changing the above logic
set fso=CreateObject("Scripting.FileSystemObject")
WorkingDirectory = fso.GetParentFolderName(Wscript.ScriptFullName)
DataFile = WorkingDirectory&"\datafile.txt"
If fso.FileExists(DataFile) Then
	Set oNotepad = fso.OpenTextFile(DataFile)
	StrData = oNotepad.ReadAll
	DataArr=Split(StrData,"|")
	EmailYN = DataArr(0)
	CName = DataArr(1)
	FileName = DataArr(2)
	FileNameCounter = cInt(DataArr(3))
	SendNotificationsForTestCases = cInt(DataArr(4))
	TestCaseCounter = cInt(DataArr(5))
	TestCaseIDsList = DataArr(6)
	Total = cInt(DataArr(7))
	Pass =cInt(DataArr(8))
	Fail = cInt(DataArr(9))
	BatchTotal = cInt(DataArr(10))
	BatchPass = cInt(DataArr(11))
	BatchFail = cInt(DataArr(12))
Else
	Msgbox("Data file don't exist")
End If
